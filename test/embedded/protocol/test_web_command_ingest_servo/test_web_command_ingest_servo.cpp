...
uint64_t servo_goto_duration_us(int32_t currentAngle, int32_t targetAngle, uint16_t rate)
{
    if (rate == 0) return 0;
    const int64_t delta = static_cast<int64_t>(targetAngle) - static_cast<int64_t>(currentAngle);
...
{
    for (int i = 0; i < 16; ++i) {
        printf("test_web_command_ingest_servo bootstrap %d/16\n", i + 1);
...
void test_sequence_u_g_i_g_dispatch_order_is_preserved(void)
{
    const char *payload =
        "{\"commands\":["
        "{\"code\":\"U\",\"data\":[1,0,0,0]},"
        "{\"code\":\"G\",\"data\":[1,1,100,100]},"
        "{\"code\":\"I\",\"data\":[1,1,5000,1000]},"
        "{\"code\":\"G\",\"data\":[1,1,0,100]}"
        "]}";
    TEST_ASSERT_EQUAL(ESP_OK, WebCommandDispatcher::processPayload(payload));
    ...
    TEST_ASSERT_EQUAL_CHAR('G', gFakeDriver.callOrder[1]);
    TEST_ASSERT_EQUAL_CHAR('I', gFakeDriver.callOrder[2]);
    TEST_ASSERT_EQUAL_CHAR('G', gFakeDriver.callOrder[3]);
}

void test_sequence_u_g_i_g_dispatch_order_is_preserved_under_concurrent_drain(void)
{
    const char *payload =
        "{\"commands\":["
        "{\"code\":\"U\",\"data\":[1,0,0,0]},"
...
void test_sequence_g_sssss_g_queue_duration_totals_2p05_seconds(void)
{
    const char *payload =
        "{\"commands\":["
        "{\"code\":\"G\",\"data\":[1,1,100,100]},"
    ...
    TEST_ASSERT_EQUAL_CHAR('G', gFakeDriver.callOrder[0]);
    ...
    TEST_ASSERT_EQUAL_CHAR('G', gFakeDriver.callOrder[6]);
    ...
    assert_uint64_equal(1000000ULL, gotoOneUs, "first goto should be 1s");
    ...
    assert_uint64_equal(1000000ULL, gotoTwoUs, "second goto should be 1s");
    assert_uint64_equal(2050000ULL, totalUs, "queue G,S,S,S,S,S,G total should be 2.05s");
}

void test_sequence_g_sssss_g_runtime_duration_is_2p05_seconds(void)
{
    ServoDriver *servo = ServoDriver::getInstance();
    CommandLayer::getInstance();
    CommandLayer::driver = servo;

    if (!gServoIoStarted) {
        servo->isrStartIoDriver();
        gServoIoStarted = true;
        vTaskDelay(pdMS_TO_TICKS(50));
    }

    TaskHandle_t motorTask = xTaskGetHandle("motorloop");
    TEST_ASSERT_NOT_NULL_MESSAGE(motorTask, "motorloop task handle is null");
    const eTaskState motorTaskState = eTaskGetState(motorTask);
    TEST_ASSERT_NOT_EQUAL_MESSAGE(eDeleted, motorTaskState, "motorloop task was deleted");

    OpBuffer::getInstance()->reset();
    servo->abortCommand(1);
    vTaskDelay(pdMS_TO_TICKS(20));

    const char *payload =
        "{\"commands\":["
        "{\"code\":\"G\",\"data\":[1,0,100,100]},"
        "{\"code\":\"S\",\"data\":[1,1,10,1000]},"
        "{\"code\":\"S\",\"data\":[1,1,10,1000]},"
        "{\"code\":\"S\",\"data\":[1,1,10,1000]},"
        "{\"code\":\"S\",\"data\":[1,1,10,1000]},"
    ...
    if (!started) {
        TEST_ASSERT_FALSE_MESSAGE(OpBuffer::getInstance()->isEmpty(1), "runtime G,S,S,S,S,S,G queue drained without entering running state");
        TEST_FAIL_MESSAGE("runtime G,S,S,S,S,S,G did not start");
    }

    bool completed = false;
    for (int i = 0; i < 1000; ++i) {
        if (!servo->isMotorRunning(1) && OpBuffer::getInstance()->isEmpty(1)) {
            completed = true;
            break;
        }
        vTaskDelay(pdMS_TO_TICKS(10));
    ...
    if (!completed) {
        ...
        snprintf(
            status,
            sizeof(status),
            "runtime G,S,S,S,S,S,G incomplete: running=%d queueEmpty=%d taskState=%d",
            servo->isMotorRunning(1) ? 1 : 0,
    ...
    TEST_ASSERT_TRUE_MESSAGE(elapsedUs >= 1950000ULL, "runtime G,S,S,S,S,S,G duration is shorter than expected");
    TEST_ASSERT_TRUE_MESSAGE(elapsedUs <= 2350000ULL, "runtime G,S,S,S,S,S,G duration is longer than expected");
}
...
extern "C" void app_main(void)
{
    ...
    RUN_TEST(test_sequence_g_sssss_g_runtime_duration_is_2p05_seconds);
    UNITY_END();
}
